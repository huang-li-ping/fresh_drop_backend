<template>
            ...
            <div class="d-flex justify-content-between">
                ...
                <div class="create">
                    <button class="btn btn-primary create-btn" @click="openModal(null)" type="button"
                        style="margin-left: auto; color: #fff">新增結果</button>
                </div>
            </div>
    ...
    <div class="show_modal d-flex flex-column align-items-start gap-2" v-if="showModal">
        ...
        <label for="id">結果編號:<input type="text" :value="newData.id || gameIdNum()" id="id" disabled="disabled"></label>

        <label for="personality">人格種類: <input type="text" :value="newData.personality" id="personality"> </label>

        <label for="" v-for="(item, index) in newData.productLists" :key="index">推薦菜色{{ index + 1 }}:
            <select v-model="item.name">
                <option v-for="(item, index) in productName" :key="index" :value="item.name">{{ item.name }}</option>
            </select>
        </label>


        <label for="txt" style="display: flex;">分析結果:
            <textarea name="" id="txt" cols="30" rows="6">{{ newData.txt }}</textarea>
        </label>
